...
import functools
from typing import Tuple, Generator, ClassVar, Type
...
def apply_with_relevant_kwargs(
    svc, fn, kwargs, ignore_when_missing_required_keys=False
):
    ...
    if ignore_when_missing_required_keys and not frozenset(kwargs.keys()).issuperset(
        required_keys
    ):
        return
    else:
...
def get_parameter_shapes(
    service_client, *operation_names: str
) -> Generator[Tuple[str, botocore.model.Shape], None, None]:
